...
class _NoHomeLocation(Exception):
    """Raised when the empty query has no HA-home coordinates to fall back on."""
...
class NoaaTidesPlusConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if query.isdigit() and len(query) == 7:
                try:
                    return await self._create_from_station_id(query)
    ...
    async def _prepare_candidates(self, query: str) -> None:
        ...
        try:
            stations = await list_tide_stations(session)
        except ApiError:
